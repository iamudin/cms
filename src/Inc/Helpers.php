...
function getWebDisk($host = false)
{
    ...
    return new \Udiko\Cms\Inc\Disk($host ? $host : false);
}
...
if (!function_exists('cache_content_initial')) {
    function cache_content_initial()
    {
        ...
        if (!cache()->has('option_' . request()->getHttpHost())) {
            recache_option();
        }
    }
}
    ...
    function regenerate_cache()
    {
        ...
        foreach ($post_type as $row) {
            ...
            \Illuminate\Support\Facades\Cache::rememberForever($row, function () use ($row, $with) {
                return \Udiko\Cms\Models\Post::with($with ?? 'user')->whereType($row)->whereStatus('publish')->latest('created_at')->get();
            });
            if (get_module($row)->form->category) {
                ...
                \Illuminate\Support\Facades\Cache::rememberForever('category_' . $row, function () use ($row) {
                    return \Udiko\Cms\Models\Category::with('posts')->whereType($row)->whereStatus('publish')->latest('created_at')->get();
                });
            }
        }
    }
...
if (!function_exists('recache_option')) {
    function recache_option()
    {
        ...
        \Illuminate\Support\Facades\Cache::rememberForever('option', function () {
            return \Udiko\Cms\Models\Option::get();
        });
    }
}
    ...
    function get_option($val = false, $host = false)
    {
        if ($val) {
            if ($host) {
                ...
                return Udiko\Cms\Models\Option::whereName($val)->first();
            }
            ...
            return $c ? (isset($c['autoload']) && $c['autoload'] == 1 ? $c['value'] : (Udiko\Cms\Models\Option::whereName($val)->first()?->value ?? null)) : null;
        }
        return '/';
    }
}
